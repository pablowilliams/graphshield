...
function GraphCanvas({ selected, onSelect, compact = false }: { selected?: string; onSelect?: (id: string) => void; compact?: boolean }) {
  ...
  return (
    <div className={`graph-canvas ${compact ? "compact" : ""}`} role="img" aria-label="Network graph. Use the equivalent table to inspect every result.">
...
function AppShell({ screen, setScreen, children, completed }: { screen: Screen; setScreen: (s: Screen) => void; children: React.ReactNode; completed: number }) {
  return (
    <div className="app-shell">
      <header className="topbar"><Brand /><div className="topbar-right"><button className="icon-button" aria-label="Help">?</button><button className="avatar" aria-label="Open profile">MP</button></div></header>
...
function Landing({ enter }: { enter: () => void }) {
  return <div className="landing">
    <nav className="landing-nav"><Brand /><div><a href="#workflow">How it works</a><a href="#trust">Built for evidence</a><button className="button ghost">View architecture</button></div></nav>
    ...
    <section className="trust-strip" id="trust"><span>DETERMINISTIC DEMO</span><span>•</span><span>EXPLAINABLE RESULTS</span><span>•</span><span>AUDITABLE RUNS</span><span>•</span><span>FORMULA-SAFE EXPORTS</span></section>
...
function SourceScreen({ next }: { next: () => void }) {
  const [tab, setTab] = useState<"demo"|"csv">("demo"); const [uploaded, setUploaded] = useState<string[]>([]); const input = useRef<HTMLInputElement>(null);
  const upload = (event: ChangeEvent<HTMLInputElement>) => setUploaded(Array.from(event.target.files || []).filter(f => f.name.endsWith(".csv") && f.size <= 25_000_000).map(f => f.name));
  return <><PageHeader eyebrow="STEP 1 OF 4 · SOURCE" title="Choose your evidence" description="Start with a prepared investigation or upload your own transaction tables." />
    ...
    {tab === "demo" ? <div className="source-layout"><section className="panel featured-source"><div className="source-cover"><span className="cover-label">DEMO CASE · GS-0926</span><div className="case-nodes"><i/><i/><i/><i/><i/></div></div><div className="source-content"><div><StatusPill tone="success">Ready to investigate</StatusPill><h2>September card network</h2><p>A deterministic synthetic dataset containing a transfer ring, shared devices, a mule hub, and a legitimate high-volume merchant control.</p></div><div className="source-stats"><div><strong>42K</strong><span>Rows</span></div><div><strong>4</strong><span>Tables</span></div><div><strong>5</strong><span>Planted patterns</span></div></div><button className="button primary" onClick={next}>Use this evidence <span>→</span></button></div></section>
      <aside className="panel case-notes"><div className="eyebrow">CASE NOTES</div><h3>What’s been planted?</h3>{["14-account transfer ring","Mule hub with 30 senders","Shared-device clusters","Bridge between two groups","Merchant false-positive control"].map((x,i)=><div className="note-row" key={x}><span>0{i+1}</span><p>{x}</p></div>)}<div className="privacy-note"><strong>✓ Safe by design</strong><p>All entities are fictional. Seed 90426 makes every run repeatable.</p></div></aside></div> : <section className="panel upload-panel"><button className="dropzone" onClick={()=>input.current?.click()} onKeyDown={(e: KeyboardEvent)=>e.key === "Enter" && input.current?.click()}><span className="upload-icon">↑</span><strong>Drop CSV files here or choose files</strong><small>CSV only · up to 25 MB each · files expire after 24 hours</small></button><input ref={input} type="file" accept=".csv,text/csv" multiple hidden onChange={upload}/>{uploaded.map(name=><div className="file-row" key={name}><span>▤</span><div><strong>{name}</strong><small>Ready to profile</small></div><StatusPill tone="success">Valid</StatusPill></div>)}<div className="panel-actions"><span>Your data is processed locally in this portfolio demo.</span><button className="button primary" disabled={!uploaded.length} onClick={next}>Profile source <span>→</span></button></div></section>}
  </>;
}
...
function RunningScreen({ algorithm, done, cancel, back }: { algorithm: Algorithm; done: () => void; cancel: () => void; back: () => void }) {
  ...
  return <div className="run-screen"><div className="run-orbit"><div className="orbit orbit-one"/><div className="orbit orbit-two"/><span>{cancelled ? "×" : `${progress}%`}</span></div><StatusPill tone={cancelled?"danger":"active"}>{cancelled?"Cancelled":"Analysis in progress"}</StatusPill><h1>{cancelled ? "Run cancelled safely" : analyses.find(a=>a.id===algorithm)?.title}</h1><p>{cancelled ? "No results were written and temporary graph resources were cleaned." : "You can leave this page. Progress and results are saved automatically."}</p>
    ...
    <div className="run-meta"><span>Run ID <code>run_f7a91c</code></span><span>Started just now</span><span>Attempt 1</span></div>{!cancelled ? <button className="button danger-button" onClick={()=>{setCancelled(true);cancel()}}>Cancel run</button> : <button className="button primary" onClick={back}>Return to analysis</button>}</div>;
}

function ResultsScreen({ algorithm, goSupport }: { algorithm: Algorithm; goSupport: () => void }) {
  ...
  const exportCsv=()=>{const safe=(v:string)=>/^[=+\-@]/.test(v)?`'${v}`:v;const csv=["# GraphShield export","# run_id,run_f7a91c","# source_checksum,a93f0c2e","rank,entity_id,component_id,component_size,risk_band,reason",...rows.map(r=>[r.rank,safe(r.id),r.group,r.size,r.risk,`\"${r.reason}\"`].join(","))].join("\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));a.download="graphshield-run_f7a91c.csv";a.click();URL.revokeObjectURL(a.href)};
  return <><PageHeader eyebrow="RUN_F7A91C · COMPLETED" title={algorithm === "WCC" ? "A connected cluster deserves review" : algorithm === "PAGERANK" ? "Structurally important accounts ranked" : "A four-hop connection was found"} description="GraphShield found evidence worth investigating—not proof of fraudulent activity." action={<div className="header-actions"><button className="button ghost" onClick={goSupport}>View run log</button><button className="button primary" onClick={exportCsv}>Export CSV ↓</button></div>} />
...
export function GraphShieldApp() {
  ...
  if(screen==="landing") return <Landing enter={()=>advance("source",0)} />;
  let content:React.ReactNode;
  if(screen==="source")content=<SourceScreen next={()=>advance("mapping",1)}/>;
  ...
  else if(screen==="analyze")content=<AnalyzeScreen run={a=>{setAlgorithm(a);setScreen("running")}}/>;
  else if(screen==="running")content=<RunningScreen algorithm={algorithm} cancel={()=>setCompleted(3)} back={()=>setScreen("analyze")} done={()=>advance("results",4)}/>;
  else if(screen==="results")content=<ResultsScreen algorithm={algorithm} goSupport={()=>setScreen("support")}/>;
  else if(screen==="history")content=<HistoryScreen select={()=>advance("results",4)}/>;
  else content=<SupportScreen/>;
  return <AppShell screen={screen} setScreen={setScreen} completed={completed}>{content}</AppShell>;
}
