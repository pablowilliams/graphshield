...
function Landing({ enter }: { enter: () => void }) {
  return <div className="landing">
    <nav className="landing-nav"><Brand /><div><a href="#product">Product</a><a href="#workflow">Workflow</a><a href="#architecture">Architecture</a><button className="button primary" onClick={enter}>Open workspace</button></div></nav>
    <main>
      <section className="hero" id="product"><div className="hero-copy"><div className="kicker">Graph analysis for fraud operations</div><h1>See the network behind suspicious activity.</h1><p>Turn transaction files into a reviewable map of accounts, devices, and infrastructure. Start with a question, inspect the evidence, and export the result.</p><div className="hero-actions"><button className="button primary large" onClick={enter}>Open sample investigation <span>→</span></button><span className="no-signup">Synthetic case · no account required</span></div><div className="hero-proof"><span><b>42,000</b> source rows</span><span><b>4,800</b> projected entities</span><span><b>8.4s</b> sample run</span></div></div>
        <div className="product-window" aria-label="GraphShield investigation workspace preview"><div className="window-bar"><div><i/><i/><i/></div><span>September card network</span><StatusPill tone="success">Saved</StatusPill></div><div className="window-body"><aside><strong>Case overview</strong><span className="active">Network</span><span>Entities</span><span>Evidence</span><span>Run log</span><small>CASE GS-0926</small></aside><section><div className="workspace-toolbar"><div><b>Shared infrastructure</b><span>9 accounts · 3 signals</span></div><button>Export</button></div><GraphCanvas compact selected="A-1047" /><div className="finding-row"><span className="risk-indicator">High</span><div><strong>A-1047 bridges two account groups</strong><p>Connected through device D-928 and IP 185.199.110.4</p></div><button onClick={enter}>Review →</button></div></section></div></div>
      </section>
      <section className="trust-strip" id="trust"><span>CSV ingestion</span><span>Identity-aware access</span><span>Audited runs</span><span>Neo4j GDS</span><span>Safe exports</span></section>
      <section className="workflow-section" id="workflow"><div><span className="section-label">Investigation workflow</span><h2>Designed around the work analysts actually do.</h2><p>GraphShield keeps setup, computation, and interpretation in one traceable case.</p></div><div className="workflow-cards">{[["Import evidence","Profile CSV files and flag quality issues before analysis."],["Define the model","Map columns to entities and relationships with a plain-language preview."],["Run an investigation","Find connected groups, influential accounts, or the shortest permitted path."],["Review the evidence","Move between the network, ranked records, and an immutable execution log."]].map(([t,d],i)=><article key={t}><span>{String(i+1).padStart(2,"0")}</span><div><h3>{t}</h3><p>{d}</p></div></article>)}</div></section>
      <section className="architecture-section" id="architecture"><div className="architecture-copy"><span className="section-label">System design</span><h2>A serious workflow needs more than a convincing screen.</h2><p>Uploads are profiled and retained temporarily. Every run gets a stable identifier, append-only events, and a cleanup record. The production adapter supports PostgreSQL worker leases and Neo4j Graph Data Science.</p></div><div className="architecture-list">{[["01","Ingest","CSV validation, type profiling, checksums, 24-hour object retention"],["02","Model","Versioned entity mappings with explicit validation warnings"],["03","Compute","WCC, PageRank, and shortest-path execution through Neo4j GDS"],["04","Review","Ranked findings, accessible tables, audit logs, and safe CSV exports"]].map(([n,t,d])=><div key={n}><span>{n}</span><strong>{t}</strong><p>{d}</p></div>)}</div></section>
    </main>
    <footer className="landing-footer"><Brand/><p>Portfolio demonstration using deterministic synthetic data.</p><button className="text-button" onClick={enter}>Open the workspace →</button></footer>
  </div>;
}
